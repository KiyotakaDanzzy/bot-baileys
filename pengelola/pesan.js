...
async function tanganiPesanMasuk(sock, m) {
    ...
    const isAiQuery =
        ...
        body.toLowerCase().startsWith("!pikir");
    ...
    const prosesPerintah = async () => {
        try {
            ...
            await sock.sendMessage(jid, {
                text: "Terjadi kesalahan internal saat memproses permintaanmu.",
            });
        }
    };
    ...
    if (isGroup) {
        ...
        try {
            if (!sock.user?.id) {
                console.log(
                    "Menerima pesan grup tetapi ID bot belum siap, mengabaikan...",
                );
                return;
            }
            ...
            if (botParticipant && (botParticipant.admin === "admin" || botParticipant.admin === "superadmin")) {
                ...
                console.log(
                    `Bot bukan admin di grup "${groupMeta.subject}", command diabaikan.`,
                );
            }
        } catch (e) {
        ...
        if (isCommand) {
            ...
            if (!sesiAktif[jid]) {
                await sock.sendMessage(
                    jid,
                    {
                        text: "Perintah tidak dikenali. Silakan gunakan command yang valid.\n\nKetik */menu* untuk melihat daftar perintah.",
                    },
                    { quoted: msg },
                );
            }
